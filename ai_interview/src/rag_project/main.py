...
def load_prompts():
    ...
    try:
        with open("configurations/prompts.yaml", encoding="utf-8") as f:
            return yaml.safe_load(f)
    except FileNotFoundError:
        print(
            "Error: configurations/prompts.yaml not found. Creating fallback prompts."
        )
        return {
...
def create_personalized_opening(
    questionnaire_context: str, profile: dict[str, Any]
) -> str:
    ...
    try:
        ...
        product_info = (
            profile.get("product_name", "") or profile.get("elevator_pitch", "")
            if profile
            else ""
        )
        ...
        if org_name and industry:
            message_parts.append(
                f"Thank you for completing the comprehensive questionnaire! I can see that {org_name} operates in the {industry.lower()} sector."
            )
        elif org_name:
            message_parts.append(
                f"Thank you for completing the comprehensive questionnaire! I have detailed information about {org_name}."
            )
        elif industry:
            message_parts.append(
                f"Thank you for completing the comprehensive questionnaire! I can see you're working in the {industry.lower()} industry."
            )
        else:
            message_parts.append(
                "Thank you for completing the comprehensive questionnaire! I have detailed information about your organization and project."
            )

        # Add product context if available
        if product_info:
            message_parts.append(
                f"I understand you're developing {product_info.lower()}."
            )
        ...
        if key_values and len(key_values) > 0:
            if len(key_values) == 1:
                message_parts.append(
                    f"I noted that {key_values[0]} is a key priority for your organization."
                )
            elif len(key_values) == 2:
                message_parts.append(
                    f"I noted that {key_values[0]} and {key_values[1]} are key priorities for your organization."
                )
            else:
                values_str = ", ".join(key_values[:2]) + f", and {key_values[2]}"
                message_parts.append(
                    f"I noted that {values_str} are key priorities for your organization."
                )
        ...
        if compliance_drivers and len(compliance_drivers) > 0:
            if len(compliance_drivers) == 1:
                message_parts.append(
                    f"You also need to address {compliance_drivers[0]} compliance requirements."
                )
            elif len(compliance_drivers) == 2:
                message_parts.append(
                    f"You also need to address {compliance_drivers[0]} and {compliance_drivers[1]} compliance requirements."
                )
            else:
                compliance_str = (
                    ", ".join(compliance_drivers[:2]) + f", and {compliance_drivers[2]}"
                )
                message_parts.append(
                    f"You also need to address {compliance_str} compliance requirements."
                )
...
try:
    EMBED = HuggingFaceEmbeddings(
        model_name="BAAI/bge-base-en", encode_kwargs={"normalize_embeddings": True}
    )
except Exception as e:
    print(f"Warning: Could not initialize embeddings: {e}")
    EMBED = None

COL_DIR = Path("./data/vector_store")
...
@dataclass
class GraphState:
    conversation_history: list[AIMessage | HumanMessage] = field(default_factory=list)
    ...
    databases_available: dict[str, bool] = field(
        default_factory=lambda: {"risks": False, "papers": False}
    )
    # New field for external questionnaire data
    external_questionnaire_data: dict[str, Any] | None = None
    questionnaire_context: str | None = None
...
def load_mit_risks_database() -> bool:
    ...
    try:
        ...
        if not sheet_url:
            print(
                "⚠️  MIT_RISKS_SHEET_URL not found in environment. Skipping MIT risks."
            )
            return False
...
def predefined_interview(state: GraphState):
    ...
    try:
        ...
        if state.external_questionnaire_data:
            ...
            transformed_profile = transform_ui_data_to_rag_profile(
                state.external_questionnaire_data
            )
            state.profile = transformed_profile

            # Create context string for freeform interview
            state.questionnaire_context = get_questionnaire_context_string(
                state.external_questionnaire_data
            )

            # Persist artifacts to logging/*.txt for debugging/inspection
            try:
                logging_dir = Path("data/output/logging")
                logging_dir.mkdir(parents=True, exist_ok=True)

                # Write transformed profile as pretty JSON
                with open(
                    logging_dir / "transformed_profile.txt", "w", encoding="utf-8"
                ) as f:
                    f.write(
                        json.dumps(transformed_profile, indent=2, ensure_ascii=False)
                    )

                # Write questionnaire context string
                with open(
                    logging_dir / "questionnaire_context.txt", "w", encoding="utf-8"
                ) as f:
            ...
            state.ai_messages = [
                AIMessage(
                    content="""Thank you for completing the comprehensive questionnaire! I have detailed information about your organization, values, compliance requirements, and technical specifications.
            ...
            print(
                f"✅ Successfully processed questionnaire data with {len(transformed_profile)} sections"
            )
            return state
...
try:
    freeform_prompt = ChatPromptTemplate.from_template(
        prompts.get("freeform_interview", "Please tell me more about your needs.")
    )
    ...
    freeform_prompt = ChatPromptTemplate.from_template(
        "Please tell me more about your organization and AI needs."
    )
    summarizer_llm = None
    search_llm = None
    ...
    try:
        ...
        if not state.conversation_history:
            ...
            if state.questionnaire_context:
                ...
                personalized_opening = create_personalized_opening(
                    state.questionnaire_context, state.profile
                )
                ...
                print(
                    "⚠️  Starting basic interview mode - no questionnaire context available"
                )
                state.conversation_history.append(
                    AIMessage(
                        content="Let's discuss your AI project and responsible AI needs..."
                    )
                )
        ...
        try:
            ...
            ai_message = AIMessage(
                content="Thank you for your responses. Let's proceed to create your strategy."
            )
    ...
    try:
        ...
        try:
            if summarizer_llm:
                ...
                summarizer_prompt_text = prompts.get(
                    "summarizer",
            ...
            if state.profile is None:
                state.profile = {
                    "user_values": "Not specified",
                    "ai_understanding": "Not specified",
                }
        ...
        state.ai_messages = [
            AIMessage(
                content="Thanks! I've gathered all the necessary information. Now, I'll build a draft strategy for you."
            )
        ]
        return state
        ...
        if state.profile is None:
            state.profile = {
                "error": "Could not summarize profile",
                "user_values": "Not specified",
                "ai_understanding": "Not specified",
            }
        state.ai_messages = [
            AIMessage(content="Let's proceed to create your strategy.")
        ]
        return state
...
def retriever(state: GraphState) -> GraphState:
    ...
    try:
        ...
        if not risks_db and not papers_db:
            print(
                "⚠️  No databases available for retrieval. Continuing without evidence."
            )
            state.evidences = []
            return state
        ...
        try:
            if search_llm:
                ...
                search_prompt_text = prompts.get(
                    "search_system",
                    "Generate 2 short search queries relevant to this profile.",
                )
                profile_json = json.dumps(state.profile, indent=2)

                # Create the prompt for structured output
                full_search_prompt = (
                    f"{search_prompt_text}\n\nUser Profile:\n{profile_json}"
                )
                ...
                print(
                    f"✅ Generated {len(queries)} search queries using structured output"
                )
                ...
                search_prompt = ChatPromptTemplate.from_messages(
                    [
                        (
                            "system",
                            prompts.get("search_system", "Generate search queries"),
                        ),
                        ("human", "{p}"),
                    ]
                )
                ...
                q = llm.invoke(
                    search_prompt.format_prompt(
                        p=json.dumps(state.profile, indent=2)
                    ).to_messages()
                ).content
        ...
        for query in queries[:3]:  # Limit to 3 queries for performance
            try:
                ...
                if papers_db:
                    for doc, score in papers_db.similarity_search_with_score(
                        query, k=3
                    ):
                        evidences.append(
                            {
    ...
    try:
        ...
        writer_prompt = ChatPromptTemplate.from_messages(
            [
                (
                    "system",
                    prompts.get("writer_system", "Write a Responsible AI strategy"),
                ),
                ("human", "PROFILE:\n{prof}"),
                ("human", "EVIDENCES:\n{ev}"),
            ]
        ...
        refs = [
            f"[S{i + 1}] {e.get('metadata', {}).get('title', 'Unknown')[:80]}"
            for i, e in enumerate(state.evidences)
        ]
        ev_snips = "\n".join(
            f"[S{i + 1}] {e['content'][:250]}…" for i, e in enumerate(state.evidences)
        )  # this is a flawed solution, because the writer does not get the full context, but we'll !FIX IT LATER!

        try:
            draft = llm.invoke(
                writer_prompt.format_prompt(
                    prof=json.dumps(state.profile, indent=2), ev=ev_snips
                ).to_messages()
            ).content
            ...
            if refs and any(ref.strip() for ref in refs):
                state.strategy += "\n\nReferences:\n" + "\n".join(
                    ref for ref in refs if ref.strip()
                )
    ...
    try:
        ...
        reviewer_prompt = ChatPromptTemplate.from_messages(
            [
                (
                    "system",
                    prompts.get("reviewer_system", "Review and improve the strategy"),
                ),
                ("human", "PROFILE:\n{prof}"),
                ("human", "EVIDENCES:\n{ev}"),
        ...
        ev_snips = "\n".join(
            f"[S{i + 1}] {e['content'][:250]}…"
            for i, e in enumerate(state.evidences or [])
        )
        ...
        state.ai_messages = [
            AIMessage(
                content="✅ Your Responsible AI strategy has been completed and refined!"
            )
        ]
    ...
    try:
        ...
        if state.conversation_history:
            ...
            if (
                ...
                and "the interview is over".lower() in last_message.content.lower()
            ):
                return "summarize_profile"
...
"""
        ...
        try:
            fallback_path = Path("data/output/responsible_ai_strategy.md")
...
async def main(external_questionnaire_data=None):
    ...
    print(
        "ℹ️  Note: Warning suppressions enabled (ChromaDB telemetry, HuggingFace parallelism, macOS malloc)."
    )
    databases_available = {
    ...
    try:
        ...
        try:
            ...
            print(
                "⚠️  Workflow invocation timed out. This might indicate an issue with LLM connectivity."
            )
            ...
            print(
                "   This might be due to LLM connectivity issues or configuration problems."
            )
            return
...
async def run_with_questionnaire_data(
