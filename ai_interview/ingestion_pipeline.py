...
"""
Ingest the MIT *AI Risk Database v3* sheet (a single tab of the public "MIT AI Risks" workbook) **plus** a folder of
Responsible‑AI PDFs into two separate Chroma collections, using the LangChain Chroma wrapper.

Key points for Chroma ≥ 1.0
───────────────────────────
* The old `chromadb.Client(Settings(persist_directory=…))` pattern is deprecated.
* Persistence is achieved by constructing a **`PersistentClient`** and passing `path="./vector_store"`.
* There is **no `client.persist()` method** any more—data is written automatically on every upsert.

Prerequisites
-------------
```bash
pip install pandas requests sentence-transformers chromadb==1.* PyPDF2 python-dotenv
```

Environment (.env or shell)
---------------------------
```dotenv
# The Google‑Sheet link while the "AI Risk Database v3" tab is active (it keeps the gid parameter)
MIT_RISKS_SHEET_URL="https://docs.google.com/spreadsheets/d/1abcDEFgHiJkLmnOpQrStu/edit#gid=1432174531"

# Directory with your PDF corpus
PAPERS_DIR=/abs/path/to/papers_folder

# Where Chroma should keep its duckdb/parquet files
