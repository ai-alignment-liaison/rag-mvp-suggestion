...
class QuestionnaireTransformer:
    ...
    def __init__(self, yaml_config_path: str = None):
        ...
        # Allow override via env var; default to project-relative path
        default_path = Path("../../UI-Experimentation5/public/config/onboarding.yaml")
        env_path = os.getenv("ONBOARDING_YAML_PATH")
        self.yaml_path = (
            Path(env_path)
            if env_path
            else (Path(yaml_config_path) if yaml_config_path else default_path)
        )
    ...
    def _load_yaml_config(self):
        ...
        try:
                        ...
                        for field in group.get("fields", []):
                            ...
                            if key:
                                ...
                                if options and isinstance(options[0], dict):
                                    ...
                                    options = [
                                        opt.get("label", opt.get("value", str(opt)))
                                        for opt in options
                                    ]
            ...
            print(
                f"✅ Loaded {len(self.question_mappings)} question mappings from YAML"
            )
    ...
    def _organize_data_by_sections(self, values: dict[str, Any]) -> dict[str, Any]:
        ...
        for key, value in values.items():
            ...
            if mapping:
                ...
                section_key = self._normalize_section_name(
                    mapping.section_title or "General"
                )
    ...
    def _format_field_value(
        self, mapping: QuestionMapping, value: Any
    ) -> dict[str, Any]:
        ...
        if mapping.field_type == "checkboxes" and isinstance(value, list):
            ...
            formatted_value["answer_summary"] = (
                f"Selected {len(value)} options: {', '.join(value)}"
            )
                ...
                if isinstance(section_data, dict):
                    for question, answer in section_data.items():
                        context_parts.append(
                            f"  • {question}: {self._format_answer_for_context(answer)}"
                        )
                else:
